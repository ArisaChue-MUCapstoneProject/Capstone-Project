...
export default function ShoppingCart(props) {
  ...
  const handleOnSubmitCartForm = () => {
    clearErrorForm()
    // error handling for when form is filled out
    if (curCategory == basicCategories[0] || curUnit == basicUnits[0] || shoppingCartForm.name.length == 0 || shoppingCartForm.quantity.length == 0) {
      let errorStrings = ["We are missing:"]
      shoppingCartForm.name.length == 0 && errorStrings.push("Name")
      shoppingCartForm.quantity.length == 0 && errorStrings.push("Quantity")
